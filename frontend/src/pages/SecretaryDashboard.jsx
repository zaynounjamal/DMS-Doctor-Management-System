    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
                <div className="p-8 text-center text-gray-500 dark:text-gray-400">Loading Dashboard...</div>
            </div>
        );
    }
                    ...
                    <div className="p-6">
                        {activeTab === 'overview' && (
                            <div className="space-y-12">
                                <div>
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Waiting Room (Checked In)</h3>
                                    <WaitingRoomLoader selectedDoctor={selectedDoctor} />
                                </div>
                                
                                <div>
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6 underline decoration-indigo-500 underline-offset-8">
                                        {selectedDoctor ? `Dr. ${doctors.find(d => d.id == selectedDoctor)?.fullName}'s Off Days` : "Doctors' Off Days"}
                                    </h3>
                                    {offDays.length === 0 ? (
                                        <p className="text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-6 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 text-center">No upcoming off days.</p>
                                    ) : (
                ...
                {step === 1 && (
                    <div className="space-y-4">
                        ...
                        <div className="max-h-60 overflow-y-auto border rounded divide-y">
                            {searchResults.map(p => (
                                <div key={p.id} className="p-2 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer flex justify-between"
                                     onClick={() => { setSelectedPatient(p); setStep(2); }}>
                                    <span>{p.fullName}</span>
                                    <span className="text-gray-500 dark:text-gray-400 text-sm">{p.phone}</span>
                                </div>
                            ))}
                            {searchResults.length === 0 && query && <div className="p-2 text-gray-500 dark:text-gray-400">No results found.</div>}
                        </div>
                        <div className="text-center pt-2 border-t">
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">New patient?</p>
                ...
                {step === 2 && selectedPatient && (
                    <form onSubmit={handleCreateAppt} className="space-y-4">
                        ...
                        <div className="flex justify-end gap-2 pt-4">
                            <button type="button" onClick={() => setStep(1)} className="text-gray-600 dark:text-gray-300 px-4 py-2">Back</button>
